...
const EditProduct = () => {
  ...
  const [loading, setLoading] = useState(false); // Trạng thái khi đang nhấn "Cập nhật"
  const [fetching, setFetching] = useState(true); // Trạng thái khi đang tải dữ liệu cũ
  ...
  const [formData, setFormData] = useState({
    ...
    stock: 0,
    isFeatured: false,
    images: [] // Chứa các FILE mới người dùng chọn thêm
  });
...
const handleSubmit = async (e) => {
  ...
  const removeImage = (index) => {
    // Lưu ý: Logic này sẽ xóa ảnh khỏi danh sách hiển thị
    const updatedPreviews = previews.filter((_, i) => i !== index);
    setPreviews(updatedPreviews);

    // Nếu là file mới chọn thì xóa trong formData.images
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <p className="text-slate-500 font-bold">Đang tải dữ liệu sản phẩm...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8 w-full bg-slate-50 min-h-screen">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight">Chỉnh sửa sản phẩm</h1>
          <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1">ID: {id}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="font-black text-slate-800 uppercase text-xs tracking-widest">Thông tin chi tiết</h3>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full px-5 py-4 bg-slate-50 border border-transparent rounded-2xl outline-none focus:bg-white focus:border-teal-200 transition-all text-sm font-bold text-slate-800" 
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full px-5 py-4 bg-slate-50 border border-transparent rounded-2xl outline-none focus:bg-white focus:border-teal-200 transition-all text-sm font-medium text-slate-600" 
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="font-black text-slate-800 uppercase text-xs tracking-widest">Hình ảnh (Sẽ thay thế toàn bộ ảnh cũ)</h3>
            </div>

            <div 
              onClick={() => fileInputRef.current.click()} 
              className="border-2 border-dashed border-slate-200 rounded-[2rem] p-12 flex flex-col items-center justify-center gap-4 bg-slate-50/50 hover:bg-teal-50/30 hover:border-teal-200 transition-all cursor-pointer group"
            >
              ...
              <div className="text-center">
                <p className="text-sm font-black text-slate-700">Tải ảnh mới lên</p>
                <p className="text-[11px] text-slate-400 font-medium mt-1">Để trống nếu muốn giữ ảnh cũ</p>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                   ...
                   <input 
                    ...
                    className="w-full pl-12 pr-5 py-4 bg-slate-50 border border-transparent rounded-2xl outline-none focus:bg-white focus:border-teal-200 transition-all text-sm font-bold text-slate-800" 
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-black uppercase tracking-widest text-slate-400 ml-1">Số lượng</label>
                <div className="relative">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full px-5 py-4 bg-slate-50 border border-transparent rounded-2xl outline-none focus:bg-white focus:border-teal-200 transition-all text-sm font-bold text-slate-800 appearance-none" 
              ...
              <label className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl cursor-pointer hover:bg-slate-100 transition-all group">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-slate-700">Nổi bật</span>
                </div>
                <input 
          ...
          <div className="grid grid-cols-2 gap-4">
            <button 
              ...
              className="py-4 bg-white border border-slate-200 rounded-2xl text-slate-600 font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 active:scale-95 transition-all shadow-sm"
            >
              Hủy
            </button>
            <button 
              type="submit" disabled={loading} 
              className="py-4 bg-teal-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-teal-200 hover:bg-teal-700 disabled:opacity-50 active:scale-95 transition-all flex items-center justify-center gap-2"
            >
              {loading ? <Loader2 className="animate-spin" size={16} /> : "Lưu thay đổi"}
            </button>
          </div>
        </div>
      </form>
